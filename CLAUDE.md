...
the same browser means the same PDF, so that comparison is the proof, and it
is faster and stricter than diffing PDFs. If the HTML changes, the change was
not a refactor, and it needs looking at rather than asserting past. The one
benign delta: theme CSS is inlined comments-and-all, so editing a CSS comment
moves HTML bytes; the proof then is markup byte-identical plus the style
block identical with comments stripped. Related, because Chromium prints are
quantized: print output snaps `position: relative` offsets and
`vertical-align` to whole CSS px, so sub-pixel nudges must ride on
`transform`, which does not snap.
...
use the first, résumé parsers the second. `scripts/text-layer.ts` rewrites
the second after Chromium prints (ligatures back to their letters, NBSP to a
...
than in a component. Third-party artwork needs in-repo provenance:
simple-icons come via the npm package (pinned exact); anything vendored as a
file gets its source and license recorded beside it or in its loader, and
the AWS icon is CC-BY-ND, so it is used verbatim with no recoloring or path
optimization. Not every built variant is released: `.releaserc.json` is the
list of what a release attaches, and README's download table must match it.
Commits follow Conventional Commits; commitlint runs in a hook and again on
every PR.

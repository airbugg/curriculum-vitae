...
- Built the second-auth layer for sensitive actions: any flow could be wrapped
  in a gate asking for Face ID, a fingerprint or a code before it runs (sending
  money, for instance), with the backend refusing the action if the gate was
  somehow bypassed. {#secondAuth}
